...
<script type="text/javascript">
	$(function() {
		...
		$("#tsys_manager_list").flexigrid({
			url : "${ctx}/questionController.do?method=query",// ajax方式对应的url地址
			type : 'post',// 数据发送方式 
			dataType : 'json',// 数据加载的类型(xml/json)        
			contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
			sortname : "questionId",//排序
			sortorder : "asc",
			useRp : true,
			radio : true,// 是否要多选框
			idProperty : 'questionId',// 多选框绑定行的id
		...
		$("#btn_query").click(function() {
			$('#tsys_manager_list').flexOptions({
				params : [{"name" : "question_questionTitle",
							"value" : $("#questionTitle").val()
						} ],
				contentType : "application/x-www-form-urlencoded; charset=UTF-8",
				newp : 1
				}).flexReload();
		...
		$("#btn_edit").click(function() {
			...
			dialogFun = sy.modalDialog({
				...
				height : 500,
				resizable : true,
				url : '${ctx}/questionController.do?method=modify&id='+id
			});
		});
		$("#preview").click(function() {
			...
			dialogFun = sy.modalDialog({
				...
				height : 300,
				resizable : true,
				url : '${ctx}/questionController.do?method=preview&id='+id
			});
		});
					...
					jQuery.messager.confirm('提示:', '你确认要删除吗?', function(event) {
						if (event) {
							...
							$(".trSelected", $("#tsys_manager_list")).each(
									function() {
										questionIds.push($(this).attr("data-id"));
									});
							
							$.ajax({
								url : "${ctx}/questionController.do",
								data : {
									method : "del",
	...
	var btn_add = function() {
		dialogFun = sy.modalDialog({
			...
			height : 500,
			resizable : true,
			url : '${ctx}/questionController.do?method=add'
		});
	};
			...
			<div id="query_area">
				...
				<table class="FormView" border="0" cellspacing="1" cellpadding="0">
					...
					<tr>
						...
						<td>
							<input type="text" name="question_questionTitle" id="questionTitle" class="text" value="">
						</td>
					</tr>
				</table>
			</div>
