...
public class TsysPostController extends HihBaseController {
	...
	public void query(HttpServletRequest request, HttpServletResponse response) {
		...
		//获取一个比表单元素map集合，里面的元素的name都是以post_开头
		Map<String, Object> param = WebUtils.getParametersStartingWith(request,"post_");
		...
		// 取出所用的父岗位的id和name    其实是把所有的数据查询出来没有进行模糊查询
		List<TsysPost> listParent = tsysPostService.selectPageTsysPostByMap(new HashMap<String, Object>());
		
		
		Map<String, String> params = new HashMap<String, String>();
		
		//迭代没有进行模糊查询的map集合，并且将存入到刚刚new的map集合里面
		for (Iterator<TsysPost> i = listParent.iterator(); i.hasNext();) {
			TsysPost post = i.next();
			params.put(post.getPostId(), post.getPostName());
		}
		
		//迭代进行了模糊查询的map集合，并且将存入到刚刚new的map集合里面
